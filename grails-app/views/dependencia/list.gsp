			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="noDependencia" title="${message(code: 'dependencia.noDependencia.label', default: 'No Dependencia')}" />
				...
				<g:each in="${dependenciaInstanceList}" status="i" var="dependenciaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${dependenciaInstance.id}">${fieldValue(bean: dependenciaInstance, field: "noDependencia")}</g:link></td>
					
						<td>${fieldValue(bean: dependenciaInstance, field: "nombre")}</td>
